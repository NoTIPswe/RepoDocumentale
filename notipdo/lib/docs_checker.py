...
def _check_doc_formatting(doc: model.Document) -> Tuple[bool, str]:
    ...
    try:
        skipped_generated = 0
        for file_path in doc.subfiles_paths.union({doc.source_path}):
            if _is_yaml_generated_typst(file_path):
                skipped_generated += 1
                continue
            subprocess.run(
        ...
        if skipped_generated:
            info += f" (skipped {skipped_generated} generated files)"
        passed = True
...
def _is_yaml_generated_typst(file_path: Path) -> bool:
    generated_header = "// Generated by notipdo YAML prebuild. Do not edit manually."
    try:
