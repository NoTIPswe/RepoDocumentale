...
def migrate_all(
    ...
    uc_stats = migrate_ucs(
        analisi_dir=analisi_dir, data_root=data_root, order_file=order_file
    )
    req_stats = migrate_requirements(
        analisi_file=analisi_dir / "analisi_requisiti.typ",
        data_root=data_root,
        order_file=order_file,
    )
    test_stats = migrate_tests(
        pq_file=pq_dir / "piano_qualifica.typ",
        data_root=data_root,
        order_file=order_file,
    )
    return {"ucs": uc_stats, "requirements": req_stats, "tests": test_stats}


def cleanup_poc(repo_root: Path = Path(".")) -> list[Path]:
    removed: list[Path] = []

    targets = [
        repo_root / "notipdo" / "poc_uml_gen.py",
        repo_root / "notipdo" / "poc_sample_ucs.yaml",
    ]
    ...
    poc_output = repo_root / "notipdo" / "poc_output"
    if poc_output.exists() and poc_output.is_dir():
        for child in sorted(poc_output.glob("**/*"), reverse=True):
            if child.is_file():
                child.unlink()
        for child in sorted(poc_output.glob("**/*"), reverse=True):
            if child.is_dir():
                child.rmdir()
        poc_output.rmdir()
        removed.append(poc_output)

    examples_dir = repo_root / ".schemas" / "examples"
    if examples_dir.exists() and examples_dir.is_dir():
        for child in sorted(examples_dir.glob("**/*"), reverse=True):
            if child.is_file():
                child.unlink()
        for child in sorted(examples_dir.glob("**/*"), reverse=True):
            if child.is_dir():
