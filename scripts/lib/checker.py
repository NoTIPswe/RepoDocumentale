...
def pr_check(
    ...
    try:
        ...
        head_docs_map = {
            doc.doc_dir_path: doc
            for doc in docs_factory.create_documents(raw_head_docs)
        }
    ...
    try:
        scanner_base = git_scanner.GitScanner(
            repo_root_path, base_branch, revision_meta_schema_path
        )
        raw_base_docs = scanner_base.discover_all_docs(docs_dir)
        base_docs_map = {
            doc.doc_dir_path: doc
            for doc in docs_factory.create_documents(raw_base_docs)
        }
    except Exception as e:
    ...
    if not head_docs_map:
        print(
            f"No documents in HEAD\nCannot define the last dir. No changes are permitted"
        )
        failure_occured = True
    else:
        all_head_doc_dirs = sorted(
            list(set(doc.doc_dir_path.parent.parent for doc in head_docs_map.values()))
        )
        ...
        if not all_head_doc_dirs:
            ...
            for doc_dir in changed_doc_dirs:
                if not doc_dir.is_relative_to(last_doc_dir):
                    print(
                        f"FAILURE: Found a file changed in {doc_dir.relative_to(repo_root_path)}"
                    )
                    failure_occured = True
            ...
            if not invalid_changes:
                ...
                print(
                    f"Changes are permitted only in the last directory: {last_doc_dir.relative_to(repo_root_path)}"
                )
    ...
    for doc_dir in changed_doc_dirs:
        ...
        if not head_doc and base_doc:
            ...
            print(
                f"OK(CREATE): {doc_name} in version {head_doc.latest_version} was created"
            )
        elif head_doc and base_doc:
            if head_doc.latest_version <= base_doc.latest_version:
                print(
                    f"FAILURE (Version): {doc_name} version's was not updated or updated wrong\n"
                )
                print(
                    f"Base: v{base_doc.latest_version}, HEAD: v{head_doc.latest_version}"
                )
                failure_occured = True
            else:
                print(
                    f"OK(Version): {doc_name} (v{base_doc.latest_version} -> v{head_doc.latest_version})"
                )
        ...
        if merge_ready and head_doc:
            ...
            if latest_entry.verifier == configs.TBD_VERIFIER:
                print(
                    f"FAILURE: {doc_name} is merge-ready but the verifier is {configs.TBD_VERIFIER}"
                )
                failure_occured = True
            else:
