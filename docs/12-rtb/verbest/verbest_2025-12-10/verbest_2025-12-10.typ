...
<<<<<<< HEAD
  ...
  #base-report.report-point(
    ...
    discussion: [L'azienda afferma che il processo di impersonificazione é valido soltanto previa approvazione del cliente, i dati decriptati sono resi disponibili soltanto previo consenso (approvazione) del cliente, al contrario, rimane possibile che l'amministratore impersonificato non possa accedere ai dati decriptati. L'attuale gerarchia proposta dal Gruppo è approvata, ma è necessario aggiungere il caso d'uso in cui l'amministratore non abbia l'accesso ad un determinato tenant.],
  ...
  #base-report.report-point(
    ...
    discussion: [Se si usano Nats e Jetstream si possono configurare i parametri, nel contesto del progetto deve essere possibile configurare la connessione, ad esempio configurando l'utilizzo di meccanismi di risposta (ACK) e non, è ragionevole pensare che nel nostro ambiente di sviluppo sia richiesta un qualche tipo di conferma ricezione. Dal punto di vista del cloud invece deve essere possibile inviare una sorta di PING per verificare lo stato di attività del gateway. ],
  )
