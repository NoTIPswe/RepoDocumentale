...
#base-report.apply-base-verbale(
  ...
  #base-report.report-point(
    ...
    discussion: [
      Il *simulatore Gateway* dovrà presentare una struttura semplice e generare dati "veritieri", condivisi tramite protocolli definiti. L’azienda ha esplicitamente richiesto semplicità e l’uso di *Docker Compose* come ambiente di simulazione.
    ],
    decisions: [
  ...
  #base-report.report-point(
    ...
    discussion: [
      M31 ha sconsigliato l’uso di Kubernetes e Google Cloud Platform perché troppo onerosi in termini di setup rispetto ai tempi del progetto. Ribadisce la preferenza di *Docker Compose* come soluzione di simulazione.
    ],
    decisions: [
      Adottare Docker Compose come ambiente infrastrutturale base del progetto. Evitare tecnologie complesse per orchestrazione cloud (mantenere un'infrastruttura semplice).
    ],
  )
  ...
  #base-report.report-point(
    ...
    discussion: [
      Tutto quanto riportato nei *requisiti opzionali* è considerato apprezzabile, ma da realizzare solo in fase finale qualora il tempo lo permetta. In particolare, M31 esprime maggiore preferenza per l'esposizione di una API e per l'utilizzo di Audit.
    ],
    decisions: [
      Soddisfare quanto riportato nei requisiti opzionali solo alla fine, qualora il tempo lo permetta, dando priorità a quelli più apprezzati dall'azienda.
    ],
  )
  ...
  #base-report.report-point(
    ...
    discussion: [
     M31 attribuisce importanza al *testbook*, documento contenente la lista di test di validazione implementati con i relativi risultati attesi. I test di scalabilità non sono obbligatori; i penetration test possono essere minimi. Gli altri test devono invece essere sviluppati adeguatamente.
    ],
    decisions: [
  ...
  #base-report.report-point(
    ...
    discussion: [
      L’azienda ha chiarito che, pur non richiedendo test di scalabilità, il sistema dovrà essere progettato per essere *scalabile orizzontalmente* in futuro.
    ],
    decisions: [
      Progettare l’architettura con componenti modulabili e replicabili, evitando soluzioni che impediscano scaling futuro.
    ],
  )
  ...
  #base-report.report-point(
    ...
    decisions: [
      Interfacciarsi con M31 come cliente durante gli incontri formali bisettimanali; consultarla come tutor durante i meeting informali settimanali o via email se servono chiarimenti operativi.
    ],
  )
  ...
  #base-report.report-point(
    ...
    discussion: [
      Viene suggerito come tool di gestione di progetto *ClickUp* o in alternativa *Jira* (più completo, però visto come meno "usabile"). 
    ],
    decisions: [
