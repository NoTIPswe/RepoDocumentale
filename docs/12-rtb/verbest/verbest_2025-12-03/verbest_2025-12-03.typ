...
#base-report.apply-base-verbale(
  ...
  #base-report.report-point(
    ...
    discussion: [L'Azienda ha chiarito che il sistema proposto (NATS) risolve il problema grazie a una cache automatica. Inoltre, permette il salvataggio di dati in modo persistente, così da poterli recuperare in caso di sospensione del servizio. L'azienda ha aggiunto un ulteriore possibile utilizzo, il protocollo buffer GRPC, che è meritevole di studio e approfondimento.],
    decisions: [Sviluppo di un sistema che garantisca il salvataggio dei dati in caso di guasto o sospensione del servizio.],
  )
  #base-report.report-point(
    discussion_point: [L'amministratore globale può impersonificare altri utenti?],
    discussion: [L'amministratore (sia esso di sistema o di tenant) può impersonificare il livello di utenza immediatamente sottostante nella gerarchia. Tuttavia, tale azione, per essere legittima, deve essere registrata nei logs.
      I dati dei Gateway possono essere visualizzabili dal sistema, tuttavia questi devono essere dissociati dall'utente di cui quei dati fanno parte.],
  ...
  #base-report.report-point(
    ...
    discussion: [L'azienda afferma che il procedimento di configurazione, nel nostro contesto, deve essere eseguito manualmente, potrebbe essere automatizzato, ma ciò avverrebbe solo in un contesto irrealistico in termini di complessità progettuale.],
    decisions: [Il procedimento di configurazione dello storage verrà eseguito manualmente dopo aver ricevuto la commissione dal cliente, risulta quindi ragionevole includere la questione dentro un caso d'uso, il Gruppo per decidere al meglio la questione invierà un'email ad M31.],
  )
  #base-report.report-point(
    discussion_point: [UC 3.1 L'amministratore di sistema può modificare il numero di risorse utilizzabili dal cliente],
    discussion: [L'Azienda afferma che la limitazione delle risorse non compete al fornitore, ma al Cliente stesso; l'utilizzo di una certa quantità di risorse viene infatti deciso e concordato tramite contratto.],
    decisions: [A seguito di un consiglio da parte dell'azienda, il Gruppo si orienta verso un modello pay-per-use, in base al quale ogni risorsa avrà una quota di pagamento visualizzabile dall'amministratore del tenant nella propria dashboard. La questione sarà comunque rivista in futuro.],
  )
