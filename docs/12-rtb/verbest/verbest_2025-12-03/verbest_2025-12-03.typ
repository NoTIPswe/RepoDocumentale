...
#base-report.apply-base-verbale(
  ...
  discussion: [L'azienda afferma che il procedimento di configurazione, nel nostro contesto, deve essere eseguita manualmente, potrebbe essere automatico in un contesto diverso definito dal Gruppo _"idilliaco"_.], decisions: [Il procedimento di configurazione dello storage verrà eseguita manualmente dopo aver ricevuto la commissione dal cliente, risulta quindi ragionevole includere la questione dentro un caso d'uso, il Gruppo per decidere al meglio la questione invierà un'email ad M31.])
