...
#base-report.apply-base-verbale(
  ...
  Il presente documento attesta formalmente che, in data *17 Dicembre 2025*, si è tenuto un incontro in remoto tramite
  ...
  L’incontro è stato dedicato alla revisione della documentazione prodotta dal gruppo durante lo Sprint 2. Ogni
  documento è stato analizzato e commentato congiuntamente al referente aziendale, in modo più o meno approfondito a
  seconda dell'entità dello stesso.

  Tale modalità ha permesso di illustrare nel dettaglio le scelte effettuate e di ricevere feedback immediati
  riguardanti principalmente i documenti nati proprio durante lo Sprint di cui la Review tratta.
][

  #base-report.report-point(
    discussion_point: [Analisi dei Requisiti e ritardi sulla timeline.],
    discussion: [
      Il gruppo ha esposto nel dettaglio la metodologia adottata per la stesura dei Casi d'Uso (UC). È stata evidenziata
      la scelta di procedere con una granularità molto fine (definita "quasi a passo singolo"), ritenuta indispensabile
      per far emergere i requisiti in modo esaustivo, seguendo quanto richiesto dal corso. Si è discusso di come tale
      approccio, seppur estremamente oneroso in termini di tempo, sia necessario per ridurre ambiguità future.

      Il gruppo ha quindi motivato il ritardo accumulato rispetto alla timeline prevista come conseguenza diretta di
      questa fase di analisi e comprensione delle criticità dell'approccio finora adottato.
    ],
    decisions: [
      L'Azienda ha validato il nuovo approccio proposto dal gruppo. È stato concordato che investire tempo ora nella
      definizione precisa dei casi d'uso è preferibile rispetto a dover correggere errori strutturali in fasi successive
      nel progetto.
    ],
  )
  ...
  #base-report.report-point(
    ...
    decisions: [
      La nuova pianificazione è stata approvata. L'Azienda ha inoltre invita il gruppo a trasmettere eventuali dubbi o
      richieste di chiarimento prima della pausa natalizia, al fine di garantire continuità operativa ed evitare blocchi
  ...
  #base-report.report-point(
    ...
    discussion: [
      Il gruppo ha illustrato la struttura del Piano di Qualifica e le metriche individuate. Il confronto con l'Azienda
      ha fatto emergere diversi punti di attenzione:
      - *Validità delle metriche (es. RSI)*, L'indice di stabilità dei requisiti (RSI) è stato giudicato spesso poco
      ...
      - *Usabilità vs Funzionalità*, In questa fase iniziale, le metriche di usabilità sono considerate secondarie
        rispetto alla necessità di fornire un applicativo funzionante e stabile.
      - *Applicazione al POC*, Molte metriche di processo e prodotto saranno applicate in ottica migliorativa solo per
        l'MVP, mentre per il POC si adotterà un approccio più orientato al produrre qualcosa di concreto.
    ],
    decisions: [
  ...
  #base-report.report-point(
    ...
    decisions: [
      ...
      del progetto e le contromisure adeguate per lo stato attuale del progetto.
    ],
  )
  ...
  #base-report.report-point(
    ...
    discussion: [
      La discussione si è concentrata sui deliverable attesi per il Proof of Concept (POC). L'Azienda ha delineato le
      seguenti richieste:
      - *Simulazione Gateway:* Realizzazione di una bozza di gateway (anche simulato) capace di instaurare una
        comunicazione.
      - *Sicurezza:* Implementazione di una sincronizzazione sicura e protetta verso il Cloud per il trasferimento dati.
      - *Interfacce Dati:* Esposizione di API per la richiesta dei dati, testabili tramite client standard (come Postman
        o client MQTT).
      - *Frontend:* La presenza di una dashboard grafica non è un requisito bloccante per il POC, ma è considerata un
        valore aggiunto ("nice to have").
    ],
    decisions: [
      Viene confermata la natura esplorativa del POC. Il codice prodotto potrà essere "usa e getta", focalizzato sulla
