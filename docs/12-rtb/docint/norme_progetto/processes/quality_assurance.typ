...
Il processo di *Accertamento della Qualità* (QA) ha lo scopo di garantire che i processi di lavoro e i prodotti realizzati siano conformi agli standard stabiliti e soddisfino gli obiettivi di qualità.
In particolare, l'Accertamento della Qualità si concentra sul monitoraggio continuo e sul miglioramento dei processi per prevenire l'introduzione di difetti, ponendo maggiore attenzione nei confronti del processo rispetto al prodotto.
...
#norm(
  ...
  label: <modello-pdca>,
  rationale: [
    *Perché PDCA?* In un contesto Agile con sprint brevi, è fondamentale iterare non solo sul prodotto ma anche sul metodo di lavoro. Il ciclo PDCA formalizza questo approccio.
  ]
)[
...
#norm(
  ...
  - *Metriche di Prodotto*: Misurano la qualità intrinseca degli artefatti (es. Indice Gulpease). Le metriche vengono raccolte e storicizzate al termine di ogni Sprint.
]
...
#norm(
  ...
  - *Jira Dashboard*: Per la visualizzazione in tempo reale delle metriche di processo che permettono di avere un'idea precisa di come il gruppo stia procedendo. In particolare tramite l'utilizzo di: Burndown Chart, Velocity Chart, distribuzione del carico di lavoro;
  - *GitHub Actions & `notipdo`*: Per la raccolta automatica delle metriche di prodotto. I log delle pipeline costituiscono la prova oggettiva del superamento dei controlli implementati fino a quel momento.
]
...
#activity(
  ...
  procedure: (
    (
      ...
      desc: [Definire e documentare le convenzioni per la codifica, la stesura dei documenti e le modalità operative del gruppo, aggiornando le presenti Norme di Progetto.],
    ),
    (
      name: "Automazione dei Controlli",
      desc: [Predisporre verifiche automatiche cercando di prevenire gli errori alla fonte, rendendo quindi impossibile il mancato rispetto delle norme.],
    ),
  ),
)
...
#activity(
  ...
  procedure: (
    (
      ...
      desc: [I sistemi di CI/CD raccolgono automaticamente le metriche di prodotto (es. coverage) ad ogni push/PR. `notipdo` verifica la conformità dei documenti.],
    ),
    (
      name: "Raccolta Manuale",
      desc: [Il Responsabile verifica su Jira la correttezza del tracciamento temporale e l'aggiornamento degli stati dei task, demandata comunque al singolo.],
    ),
  ),
)
