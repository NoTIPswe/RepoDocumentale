...
=== notip-data-consumer
  ...
  l'osservabilità e `/healthz` per la verifica della raggiungibilità del database. Assolve tre responsabilità architetturali:
  ...
  - *Dominio* (`internal/domain/model`, `internal/domain/port`, `internal/service`): contiene i value object puri
    ...
    e il servizio `HeartbeatTracker`, che incapsula l'intera logica di liveness dedicata ai Gateway. Questo strato non importa alcun package
    infrastrutturale.
  ...
  *Driven Port* sono interfacce invocate dal dominio, implementate dagli adapter per raggiungere l'esterno:
  ...
  #st.port-interface(
    ...
    kind: "driven",
    description: [Confine di persistenza per i record telemetrici. Permette di isolare il dominio dalla tecnologia di storage,
      consentendo la sostituzione del backend di persistenza senza alcun tipo di impatto sulla logica applicativa.],
    methods: (
  ...
  #st.port-interface(
    ...
    description: [Confine di pubblicazione degli alert infrastrutturali. Disaccoppia la logica di rilevazione offline
      dal meccanismo di distribuzione degli alert (JetStream).],
    methods: (
  ...
  #st.port-interface(
    ...
    kind: "driven",
    description: [Confine di notifica delle transizioni di stato. Permette di isolare il dominio dal protocollo di comunicazione con
      il Management API (NATS Request-Reply).],
    methods: (
      ("UpdateStatus", [Notifica una transizione di stato online/offline al Management API, servizio con accesso in scrittura al database dedicato]),
    ),
  )
  ...
  #st.port-interface(
    ...
    kind: "driven",
    description: [Confine di accesso alla configurazione degli alert. Permette al dominio di richiedere il timeout senza conoscere la
      sorgente che lo fornisce.],
    methods: (
  ...
  #st.port-interface(
    name: "ClockProvider",
    kind: "driven",
    description: [Astrazione del tempo di sistema. Consente l'iniezione di un clock deterministico nei test, eliminando
      la dipendenza diretta da `time.Now()` nella logica di dominio.],
    methods: (
  ...
  *Driving Port* sono interfacce implementate dal dominio, invocate dagli adapter per immettere eventi:
  ...
  #st.port-interface(
    ...
    kind: "driving",
    description: [Punto di ingresso per gli eventi di decommissione gateway. Permette di rimuovere il gateway dalla mappa di
      heartbeat, prevenendo falsi alert offline per gateway già conosciuti come dismessi.],
    methods: (
  ...
  #st.design-rationale(title: "Tick a tre fasi con re-validazione")[
    ...
    Se durante la fase di I/O è arrivato un nuovo messaggio dal
    gateway (il `lastSeen` è avanzato), la transizione a offline viene annullata. Questa strategia premette di minimizzare la contesa
    sul lock della mappa e previene falsi positivi causati dalla latenza delle operazioni di rete.
  ]

  #st.design-rationale(title: "Grace period all'avvio")[
    All'avvio del servizio, nessun gateway ha ancora inviato alcun tipo di telemetria: tutti risulterebbero immediatamente offline, innescando l'invio di alert "falsi". Un
    periodo di grazia configurabile sopprime gli alert durante la fase di warm-up, consentendo ai
    gateway di stabilire la connessione e inviare il primo messaggio prima che il meccanismo di heartbeat diventi
    attivo.
  ]
  ...
  #st.design-rationale(title: "Batch write con flush periodico")[
    ...
    tramite un singolo round-trip di rete. Il flush avviene al raggiungimento di una soglia o alla scadenza
    di un timer. I messaggi NATS vengono ACK-ati solo dopo la scrittura
    riuscita del batch, garantendo at-least-once delivery.
  ]
