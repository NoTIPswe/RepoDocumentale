...
#base-document.apply-base-document(
	...
	- configurazione e osservabilita (`config`, `metrics`).
	...
	#table(
		...
		[Presentation],
		[`ProvisioningController`, `OnboardRequestDto`, `OnboardResponseDto`, `ProvisioningExceptionFilter`, `AuditLogInterceptor`],
		...
		[Infrastructure],
		[`NATSRRClient`, `NATSFactoryValidator`, `NATSProvisioningCompleter`, `ForgeCSRSignerService`, `AESKeyGeneratorService`],
	...
	#table(
		...
		[1], [Validazione factory credentials],
		[Chiama `FactoryValidator.validate`; riceve `GatewayIdentity` o errori 401/409/503.],
		[2], [Firma CSR],
		[Chiama `CSRSigner.sign`; produce certificato foglia X.509 o errore 400.],
		[3], [Generazione chiave AES],
		[Chiama `AESKeyGenerator.generate`; produce 32 byte random e `version = 1`.],
		[4], [Completamento provisioning],
		[Chiama `ProvisioningCompleter.complete` con identity, chiave e `send_frequency_ms`.],
		[5], [Aggiornamento metriche],
		[Incrementa counter success/failure e osserva durate delle operazioni critiche.],
		[6], [Mapping output],
		[Restituisce `certificate`, `aeskey` (base64) e `send_frequency_ms` con status 201.],
	)
	...
	#table(
		...
		[`GatewayCSR`],
		[`pemData: string`],
		[Valida header PEM `-----BEGIN CERTIFICATE REQUEST-----` in costruzione.],
		...
		[Identita ricevuta dalla validazione Management API.],

		[`AESKey`],
		[`material: Buffer(32)`, `version: number`],
		[Espone `toBase64()` per serializzazione risposta/NATS.],

		[`SignedCertificate`],
		[`pemData: string`],
		[Certificato foglia firmato dalla CA interna.],
		...
		[Materiale CA in memoria process-wide dopo bootstrap.],
