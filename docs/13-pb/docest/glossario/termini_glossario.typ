#let terms = (
  // A
  "Acceptance Test": "Test che verifica il comportamento del sistema completo simulando scenari d'uso reali, eseguito internamente dal team prima del rilascio o congiuntamente al proponente durante le revisioni di avanzamento",
  ...
  "AES-256": "Algoritmo di cifratura simmetrica a 256 bit utilizzato per proteggere i payload telemetrici con decifratura esclusivamente client-side, garantendo la riservatezza dei dati anche verso l'infrastruttura del provider",
  "Actuator": "Dispositivo fisico o componente di controllo in grado di eseguire azioni o modificare lo stato di un sistema sulla base di segnali ricevuti, utilizzato nei sistemi IoT in sinergia con i sensori",
  ...
  "Black-box Testing": "Metodologia di test che verifica il comportamento di un componente sulla base degli input e output attesi, senza considerare la struttura interna del codice",
  ...
  "Buffer": "Memoria temporanea utilizzata per conservare dati in transito, spesso impiegata nei message broker per disaccoppiare la velocità di produzione da quella di consumo",
  "Burndown Chart": "Grafico che visualizza il progresso di uno sprint tracciando il lavoro rimanente nel tempo, permettendo di identificare ritardi o accelerazioni rispetto alla pianificazione",
  ...
  "Checklist": "Lista di controllo strutturata utilizzata nelle ispezioni per individuare sistematicamente errori specifici durante la verifica di documenti o codice",
  "CLI": "Interfaccia a riga di comando che permette di eseguire operazioni tramite testo invece che con un'interfaccia grafica",
  ...
  "Consumer": "Componente del sistema di messaggistica che riceve e processa i messaggi pubblicati su una coda o un topic da parte di un producer",
  ...
  "Decommissioning": "Procedura di dismissione di un gateway dalla piattaforma, che comprende la revoca della chiave AES, la pulizia delle risorse NATS e la pubblicazione dell'evento di decommissioning ai servizi interessati",
  "Deploy": "Processo di distribuzione e installazione del software in un ambiente di test o produzione",
  "DevContainer": "Ambiente di sviluppo isolato e riproducibile, avviato tramite contenitori, che fornisce a tutti gli sviluppatori gli stessi strumenti e dipendenze",
  ...
  "GCM": "Acronimo di Galois/Counter Mode, modalità di cifratura autenticata usata con algoritmi come AES per garantire riservatezza e integrità dei dati",
  ...
  "Goroutine": "Unità leggera di esecuzione concorrente del linguaggio Go, gestita dal runtime in modo efficiente per abilitare l'elaborazione parallela di migliaia di operazioni simultanee",
  ...
  // H
  "Handshake": "Procedura di negoziazione iniziale tra due entità di comunicazione per stabilire una connessione sicura e concordare i parametri di scambio, come avviene nel protocollo TLS",
  ...
  "IV": "Acronimo di Initialization Vector, valore casuale monouso combinato con la chiave crittografica nel cifrario AES-256-GCM per garantire che messaggi identici producano ciphertext diversi",
  ...
  "Latency": "Ritardo temporale tra il momento in cui una richiesta viene inviata e il momento in cui viene ricevuta la risposta, misurato generalmente in millisecondi",
  ...
  "Middleware": "Componente software che agisce da intermediario tra applicazioni o servizi diversi, gestendo comunicazione, trasformazione dei dati e routing delle richieste",
  "Monorepo": "Strategia di organizzazione in cui più componenti o progetti correlati convivono nello stesso repository",
  ...
  "Offuscamento": "Tecnica che rende meno leggibile o immediato un contenuto o un'informazione, pur mantenendone la funzionalità, spesso usata per ridurre l'esposizione di dati sensibili",
  ...
  "Onboarding": "Processo di registrazione e configurazione iniziale di un gateway nella piattaforma: include la validazione delle credenziali di fabbrica, la firma del certificato mTLS e la generazione della chiave AES-256 per la cifratura end-to-end",
  ...
  // P
  "Payload": "Contenuto effettivo dei dati trasmessi in un messaggio, pacchetto o richiesta HTTP, al netto degli header e dei metadati di controllo del protocollo",
  ...
  "PKCE": "Acronimo di Proof Key for Code Exchange, estensione di OAuth2 che rafforza la sicurezza del flusso di autorizzazione, soprattutto per client pubblici come applicazioni web e mobile",
  ...
  "Producer": "Componente del sistema che genera e pubblica messaggi verso un message broker o topic, per il successivo consumo da parte dei consumer",
  ...
  "RBAC": "Acronimo di Role-Based Access Control, modello di autorizzazione che assegna i permessi in base al ruolo dell'utente invece che al singolo account",
  "Release": "Versione ufficiale del software o della documentazione consegnata agli utenti o agli stakeholder, identificata da un numero di versione secondo SemVer",
  ...
  // S
  "Server-Sent Events": "Protocollo HTTP unidirezionale che permette al server di inviare aggiornamenti in tempo reale al client attraverso una connessione persistente; utilizzato per lo streaming della telemetria cifrata verso il browser",
  "SSE": "Acronimo di Server-Sent Events, vedi relativa voce",
  ...
  "Testcontainers": "Libreria per i test automatici che avvia container usa e getta durante l'esecuzione, così da simulare dipendenze esterne in modo controllato",
  "Throughput": "Quantità di messaggi o dati elaborati con successo per unità di tempo; metrica chiave per valutare la capacità di ingestione del Data Consumer nella piattaforma IoT",
  ...
  "Use Case": "Descrizione di un obiettivo dell'utente e delle interazioni necessarie per raggiungerlo, usata per chiarire cosa deve fare il sistema dal punto di vista funzionale",
  ...
  "Web Crypto API": "API nativa dei browser moderni che fornisce primitive crittografiche hardware-accelerate; utilizzata nel frontend per la decifratura AES-256-GCM dei payload telemetrici senza dipendenze esterne",
  "Web Worker": "API del browser che permette di eseguire script JavaScript in un thread separato dal thread principale, evitando blocchi dell'interfaccia durante operazioni intensive come la decifratura dei dati telemetrici",
