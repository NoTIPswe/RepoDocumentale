...
#norm(
  ...
  rationale: [
    ...
    )[*Conventional Commits*] ( _ultimo accesso: 2026-03-02_) o di rileggere la #link(
      <branching-commit-docs>,
    )[@branching-commit-docs].
  ],
)[
...
#norm(
  ...
  Ogni membro del team è tenuto a configurare il proprio ambiente locale prima del primo commit; il ramo principale deve
  essere denominato `main` (e non `master`) per conformità con le policy del repository remoto.
]
