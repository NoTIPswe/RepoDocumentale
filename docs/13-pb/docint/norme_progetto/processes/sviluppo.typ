...
#norm(
  ...
  *API Sincrone (REST / OpenAPI)*
  L'approccio adottato per la condivisione delle interfacce tra servizi è *code-first*: i servizi che espongono API
  (tutti sviluppati in NestJS) definiscono le interfacce direttamente nel codice sorgente tramite decoratori
  Swagger/OpenAPI, senza file di specifica redatti manualmente.

  La directory `notip-infra/api-contracts/` funge da registro centrale per i contratti API pubblicati dai backend. I
  servizi consumatori (frontend Angular, data-consumer Go) non dipendono da un file mantenuto a mano, ma da uno snapshot
  generato e pubblicato automaticamente dalla CI del backend.

  Il file `openapi.json` scaricato e committato nella repository di ogni servizio consumatore costituisce un *lockfile
  dell'API*: esso traccia l'esatta versione dell'interfaccia contro cui il consumer è compilato ed è un elemento di
  configurazione soggetto a versionamento (vedi @config-items).

  I servizi consumatori utilizzano tool standard per la generazione automatica di DTO, interfacce e client HTTP a
  partire dal JSON locale:
  ...
  *API Asincrone (NATS / AsyncAPI)*
  ...
  esplicitamente il ruolo (`Publisher` o `Subscriber`). Questo approccio permette ai singoli servizi di filtrare il
  documento e autogenerare esclusivamente le interfacce di proprio interesse.
...
#norm(
  ...
  I database principali del sistema (MeasuresDB e ManagementDB) devono essere creati e migrati tramite gli strumenti di
  migrazione offerti da TypeORM in NestJS. È vietato applicare modifiche agli schemi dei database in modo manuale o
  tramite strumenti alternativi.

  Le migrazioni devono essere sempre generate, committate e versionate nella repository del servizio di riferimento.
  Esse costituiscono elementi di configurazione soggetti a versionamento obbligatorio (vedi @config-items).

  Il database del simulatore (SQLite), essendo un mock a scopo di testing e sviluppo, è escluso da questo requisito: non
  si producono né si versionano migrazioni per esso.
]
...
#activity(
  ...
  output: [
    `openapi.json` aggiornato committato nella repository del servizio consumatore, client HTTP autogenerato aggiornato
  ],
  procedure: (
    (
      name: "Estrazione (Backend)",
      desc: [
        Durante la build, uno script NestJS genera offline il file `openapi.json` direttamente dai decoratori del
        codice. Il file non viene committato nella repository del backend.
      ],
    ),
    (
      name: "Pubblicazione (Push nel Registro)",
      desc: [
        La CI del backend pusha automaticamente il JSON generato nella cartella `api-contracts/` dentro `notip-infra`,
        che funge da registro centrale.
      ],
    ),
    (
      name: "Sincronizzazione (Pull del Consumatore)",
      desc: [
        Lo sviluppatore del servizio consumatore lancia uno script locale che scarica il file `openapi.json` da
        `notip-infra`, puntando a un commit SHA specifico (pinning), garantendo la riproducibilità.
      ],
    ),
    (
      name: "Lockfile",
      desc: [
        Il file JSON scaricato viene committato nella repository del consumatore. Esso funge da lockfile e traccia
        l'esatta versione dell'API contro cui il consumer è compilato.
      ],
    ),
    (
      name: "Generazione Codice",
      desc: [
        Il consumatore esegue il tool di generazione appropriato (`openapi-generator-cli` per TypeScript, `oapi-codegen`
        per Go) per autogenerare DTO, interfacce e client HTTP a partire dal JSON locale appena aggiornato.
      ],
    ),
  ),
)
