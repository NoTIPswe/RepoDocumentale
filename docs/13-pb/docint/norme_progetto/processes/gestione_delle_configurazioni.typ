...
#norm(
  ...
  - *Sito Web*: I template e gli script per la generazione del sito statico contenuti nella
    directory `site/`;
  - *Dizionari*: I file di dizionario personalizzati (es. `.hunspell/it_IT.dic`) utilizzati per
    il controllo ortografico;
  - *Schemi di validazione*: File contenuti all'interno della directory `.schemas/`;
  - *Automazione*: Il codice sorgente del tool `notipdo` e i workflow di GitHub
    (`.github/workflows/`) utilizzati nella chiusura delle PR e rilascio del sito web;
  - *File di configurazione degli ambienti* (`.env.example`): Ogni repository deve contenere
    il file `.env.example` committato, che documenta tutte le variabili d'ambiente necessarie
    all'esecuzione del servizio senza esporre valori reali. Per la politica di gestione dei
    segreti si rimanda alla @gestione-segreti;
  - *Migrazioni del database*: I file di migrazione generati da TypeORM per i database principali
    (MeasuresDB e ManagementDB) sono elementi di configurazione soggetti a versionamento
    obbligatorio. Per le norme di utilizzo dello strumento si rimanda alla @database-migrazioni;
  - *Lockfile delle API* (`openapi.json`): Il file `openapi.json` scaricato da `notip-infra` e
    committato nella repository di ogni servizio consumatore costituisce un elemento di
    configurazione. Esso rappresenta la fotografia dell'esatta versione dell'API contro cui il
    consumer è compilato (es. FE v1.0.0 sa di usare BE v0.6.7 perché lo dice il JSON). Per il
  ...
  - *Definizioni degli ambienti di sviluppo* (DevContainers): I Dockerfile base contenuti in
    `notip-infra/devcontainers/` e i file `.devcontainer/devcontainer.json` presenti in ogni
    repository sono elementi di configurazione soggetti a versionamento. Essi garantiscono la
    riproducibilità degli ambienti di sviluppo. Per la struttura e l'architettura si rimanda
    alla @devcontainers.
  - *Immagini Docker di build e rilascio* (`notip-infra/containers/`): I Dockerfile base per la
    build e il rilascio in produzione dei servizi applicativi sono elementi di configurazione
    soggetti a versionamento. Per la policy di utilizzo si rimanda alla @build-release-process.
]
...
#norm(
  ...
  Il gruppo adotta una strategia Multi-repo per garantire una netta separazione delle
  responsabilità e mantenere lineare la cronologia dei contributi. La struttura è così suddivisa:
  - *Repository Documentale*: Contiene la documentazione di progetto, il sito web e il tool
    `notipdo`;
  - *Repository PoC*: Contiene il codice sorgente per la Technology Baseline (Proof of Concept);
  - *Repository di Prodotto*: Il prodotto è sviluppato attraverso le seguenti repository distinte,
    una per servizio, secondo un'architettura a microservizi:
    - `notip-infra`: infrastruttura, configurazione di monitoring (Prometheus+Grafana), NATS,
      Nginx, devcontainer base, api-contracts e test di sistema e integrazione multi-servizio.
      È un monorepo;
    - `notip-data-api`: backend NestJS e MeasuresDB (PostgreSQL + TimescaleDB);
    - `notip-frontend`: web app Angular;
    - `notip-simulator`: simulatore completo (backend Go, frontend Angular, database SQLite).
      È un monorepo;
  ...
  *Gestione dei Monorepo*
  ...
  I repository `notip-infra` e `notip-simulator` sono strutturati come monorepo, contenendo più
  componenti con cicli di vita distinti.

  *Path Filtering*: I workflow di CI vengono triggerati esclusivamente dai commit che modificano
  file nella sottodirectory del componente interessato. Ad esempio, il workflow del frontend di
  `notip-simulator` viene attivato solo da modifiche sotto `notip-simulator/sim-dashboard/`;
  quello del backend solo da modifiche sotto `notip-simulator/simulator/`.

  *Versionamento*: Per `notip-infra` non si traccia una versione globale, in quanto funge da
  contenitore di infrastruttura condivisa. Per `notip-simulator` si adotta invece una singola
  versione globale dell'intero ecosistema simulatore. Per la politica di versionamento automatico
  tramite CI/CD si rimanda alla @versionamento-codice.

  *Struttura delle directory di test in `notip-infra`*

  La repository `notip-infra` ospita i test che coinvolgono più servizi, secondo la seguente
  struttura:
...
#norm(
  ...
  rationale: [
    La logica di avanzamento automatico si applica allo stato del branch `main` al momento del
    merge. In presenza di più commit in una singola Pull Request, vince il prefisso di peso
    maggiore. Questo approccio elimina la necessità di aggiornare manualmente i numeri di
    versione, riducendo il rischio di errori e disallineamenti tra repository.
  ],
)[
  Il versionamento del codice sorgente segue una strategia *Trunk-Based Development*: lo sviluppo
  avviene sul branch `main` tramite Pull Request piccole e frequenti, senza overhead derivante da
  strategie come GitFlow.

  Le versioni (Major, Minor, Patch) e i relativi tag Git vengono calcolati e generati
  automaticamente dalla CI/CD leggendo i prefissi dei commit secondo lo standard Conventional
  Commits. Ogni repository di prodotto avanza di versione in modo indipendente.

  Per i prefissi dei commit e il loro peso nella determinazione della versione si rimanda alla
  @branching-conventional-commits. Per le norme sulla strategia dei repository e la gestione dei
  monorepo si rimanda alla @repo-strategy.
]
...
#norm(
  ...
  Le configurazioni e i segreti di progetto sono gestiti tramite file `.env`. In fase di CI/deploy
  i valori vengono iniettati tramite GitHub Secrets.

  I file `.env` reali non vengono mai committati nel repository, in quanto esclusi tramite
  `.gitignore` come definito nella @git-ignore-policy. Ogni repository deve invece contenere
  un file `.env.example` committato, che documenta tutte le variabili necessarie all'esecuzione
  senza esporre valori reali (vedi @config-items).

  I file `.env` locali rappresentano sempre la configurazione di sviluppo (dev), mentre la
  configurazione generata da GitHub Secrets è quella di produzione (prod).

  Per il setup del file `.env` locale da parte di ogni sviluppatore si rimanda alla
  @setup-devcontainer.
]
...
#norm(
  ...
  Per correzioni minori o aggiustamenti all'ultimo commit, è vietato produrre una sequenza di
  micro-commit correttivi consecutivi (es. tre commit successivi con messaggio "fix typo"). Si
  deve invece ricorrere a:
  ...
  Questa norma vale sia per il codice sorgente sia per la documentazione. Per la documentazione
  il rimando operativo è alla @branching-commit-docs.
]
